  ...
  return (
    <PageContainer title="Coupons" subtitle="Create and manage discount campaigns">
      ...
      {showForm && (
        <Card className="border-indigo-100 animate-scale-in">
          ...
          <PrimaryButton className="mt-3" onClick={async () => {
            ...
            setForm({ code:"", message:"", active:true });
            setShowForm(false);
            load();
          }}>Save Coupon</PrimaryButton>
        </Card>
      )}
      ...
      <Card>
        ...
        {coupons.length === 0 ? (
          <EmptyState icon="🎟️" title="No coupons yet" description="Create your first coupon above." />
        ) : (
            ...
            {coupons.map(c => (
              <div key={c.id} className="flex items-center gap-3 rounded-xl border border-gray-100 bg-gray-50 p-3">
                ...
                <div className="flex gap-1.5">
                  <button
                    onClick={async () => { const code = prompt("Code", c.code); const message = prompt("Message", c.message); if(!code||!message) return; await api.put(`/admin/coupons/${c.id}`, { ...c, code, message }); load(); }}
                    className="h-8 w-8 flex items-center justify-center rounded-xl border border-gray-200 bg-white hover:border-indigo-200 hover:text-indigo-600 transition-all text-xs"
                  >✏️</button>
                  <button
                    onClick={async () => { if(!confirm("Delete?")) return; await api.delete(`/admin/coupons/${c.id}`); load(); }}
                    className="h-8 w-8 flex items-center justify-center rounded-xl border border-red-100 bg-white hover:bg-red-50 hover:text-red-600 transition-all text-xs"
                  >🗑️</button>
                </div>
              </div>
            ))}
