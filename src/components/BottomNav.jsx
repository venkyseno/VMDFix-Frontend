        ...
        {items.map(({ path, label, icon: Icon }) => {
          ...
          return (
            <button key={path} onClick={() => navigate(path)}
              className={`flex flex-col items-center gap-0.5 px-5 py-1.5 rounded-xl transition-all ${active ? "" : "text-gray-400 hover:text-gray-600"}`}
              ...
              <div className="relative flex h-7 w-7 items-center justify-center rounded-xl transition-all"
                ...
                {active && <span className="absolute -bottom-0.5 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full" style={{ background: "#1a7a4a" }} />}
              </div>
              <span className="text-[10px] font-semibold leading-none" style={active ? { color: "#1a7a4a" } : {}}>{label}</span>
            </button>
          );
        })}
      </div>
