...
import {
  Card, EmptyState, PageContainer, SectionHeader
} from "../../components/ui";
...
export default function AdminOurServicesPage() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const r = await api.get("/admin/our-services");
      setServices(r.data || []);
  ...
  const save = async () => {
    ...
    try {
      await api.post("/admin/our-services", form);
  ...
  const update = async () => {
    ...
    try {
      // Default cards have string IDs like "do1" — save as new instead of PUT
      const isDefault = typeof editId === "string" && isNaN(Number(editId));
      if (isDefault) {
        const { id, ...payload } = editData;
        await api.post("/admin/our-services", payload);
      } else {
        await api.put(`/admin/our-services/${editId}`, editData);
      }
  ...
  const del = async (id) => {
    ...
    try { await api.delete(`/admin/our-services/${id}`); load(); } catch (e) { err(e); }
  };
        ...
        {GRADIENTS.map(g => (
          <button key={g.value} type="button" onClick={() => onChange(g.value)}
            className={`h-8 w-14 rounded-lg bg-gradient-to-br ${g.value} shadow-sm transition-all
              ${value === g.value ? "ring-2 ring-blue-500 ring-offset-1 scale-110" : "hover:scale-105"}`}
            title={g.label} />
        ))}
      </div>
    </div>
      ...
      <div className="space-y-3">
        ...
        <div className="flex gap-2 pt-1">
          <button type="button" onClick={onSave}
            ...
            style={{ background: "linear-gradient(135deg,#16213e,#2563eb)" }}>
    ...
    <PageContainer title="Our Services" subtitle="Manage the 'Our Services' section on the home page">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <p className="text-xs text-gray-400">
          {isDefault ? "📌 Showing defaults — add services to override" : `${services.length} services saved`}
        </p>
      ...
      {isDefault && (
        <div className="rounded-xl bg-blue-50 border border-blue-200 px-4 py-3 text-sm text-blue-700">
          🛠️ <strong>Default services</strong> are shown on home. Add your own to override. Click <strong>✏️</strong> to edit a default and save it.
        </div>
      )}
      ...
      {showForm && (
        <ServiceForm
          ...
          isEdit={false} title="New Our Service"
        />
      )}
      ...
      <Card>
        ...
        {loading ? (
          ...
          <EmptyState icon="🛠️" title="No services yet" description="Add your first service." />
        ) : (
                ...
                <div className="flex items-center gap-3 rounded-xl border border-gray-100 bg-gray-50 p-3">
                  ...
                  <div className="flex gap-1.5 flex-shrink-0">
                    <button type="button" onClick={() => startEdit(s)}
                      className="h-8 w-8 flex items-center justify-center rounded-xl bg-white border border-gray-200 hover:border-blue-300 hover:text-blue-700 transition-all"
                      title="Edit">
                      <Pencil size={13} />
                    </button>
                    {!isDefault && (
