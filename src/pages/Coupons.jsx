        ...
        <div className="grid gap-3 sm:grid-cols-2 stagger-children">
          {coupons.map(c => (
            <div key={c.id} className="relative rounded-2xl border border-amber-200 bg-gradient-to-br from-amber-50 to-orange-50 p-4 overflow-hidden">
              <div className="absolute right-0 top-0 h-full w-16 opacity-10">
                <Tag size={80} className="text-amber-700 rotate-12 -translate-y-2 translate-x-4" />
              </div>
              <div className="relative">
                <div className="flex items-center justify-between mb-2">
                  <span className="rounded-lg bg-amber-100 border border-amber-200 px-2.5 py-1 text-xs font-bold text-amber-800 font-mono tracking-wider">
                    {c.code}
                  </span>
                  <button
