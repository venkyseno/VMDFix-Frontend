  ...
  useEffect(() => {
    api.get("/admin/cases").then(r => {
      ...
      setStats(s => ({
        ...s,
        orders: cases.length,
        revenue: cases.filter(c => c.status === "CLOSED").reduce((acc, c) => acc + Number(c.serviceAmount || 0), 0)
      }));
    }).catch(() => {});
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 stagger-children">
        <StatCard title={t("orders")} value={stats.orders} subtitle={t("live")} icon={<LayoutGrid size={16} />} tone="indigo" />
        <StatCard title={t("services")} value={stats.services} subtitle={t("catalog")} icon={<Wrench size={16} />} tone="blue" />
        <StatCard title={t("workers")} value={stats.users} subtitle={t("active")} icon={<Users size={16} />} tone="emerald" />
        <StatCard title={t("revenue")} value={`₹${stats.revenue}`} subtitle={t("today")} icon={<CreditCard size={16} />} tone="amber" />
      </div>
