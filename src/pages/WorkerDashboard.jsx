...
export default function WorkerDashboard() {
  ...
  const startWork = async (id) => {
    try { await api.post(`/cases/${id}/start-work?workerId=${user.id}`); fetchCases(); }
    catch (e) { alert(e.response?.data || "Failed"); }
  };

  const completeWork = async (id) => {
    try { await api.post(`/cases/${id}/complete-work?workerId=${user.id}`); fetchCases(); }
    catch (e) { alert(e.response?.data || "Failed"); }
  };
    ...
    <PageContainer title={`${t("worker_hello")}, ${user?.name || "Worker"} 👋`} subtitle={t("worker_assigned_jobs")}>
      ...
      {loading ? (
        <div className="space-y-3">
          {[1, 2].map(i => <div key={i} className="h-24 rounded-2xl bg-white border border-gray-100 animate-pulse" />)}
        </div>
      ) : cases.length === 0 ? (
          ...
          {cases.map(c => (
            <div key={c.id} className={`rounded-2xl border p-4 bg-white shadow-sm ${c.status === "IN_PROGRESS" ? "border-amber-200" : "border-gray-100"}`}>
              <div className="flex items-start justify-between mb-2">
                <p className="font-bold text-gray-900 text-sm">{t("case_hash")}{c.id}</p>
              ...
              <div className="flex gap-2">
                {c.status === "ASSIGNED" && (
                  <button onClick={() => startWork(c.id)}
                    className="flex items-center gap-1.5 rounded-xl bg-indigo-600 px-3 py-1.5 text-xs font-bold text-white hover:bg-indigo-700 transition-colors active:scale-95">
                    <Play size={11} />{t("start_work")}
                  </button>
                )}
                {c.status === "IN_PROGRESS" && (
                  <button onClick={() => completeWork(c.id)}
                    className="flex items-center gap-1.5 rounded-xl bg-emerald-600 px-3 py-1.5 text-xs font-bold text-white hover:bg-emerald-700 transition-colors active:scale-95">
