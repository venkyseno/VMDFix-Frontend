...
export default function Profile() {
  ...
  const [view, setView] = useState("profile"); // "profile" | "addresses"
  ...
  const handleDeleteAddress = async (id) => {
    ...
    try {
      await api.delete(`/user-flow/addresses/${id}`);
  ...
  const handleSetPrimary = async (id) => {
    try {
      await api.put(`/user-flow/addresses/${id}/primary`, { userId: user.id });
      showToast("Primary address updated!");
      loadAddresses();
    } catch (e) {
      console.error("Set primary error:", e);
      showToast(e?.response?.data?.message || e?.response?.data || "Failed to set primary");
    }
  };
  ...
  const handleUpdateAddress = async () => {
    ...
    try {
      await api.put(`/user-flow/addresses/${editingAddress.id}`, { ...editingAddress, userId: user.id });
  ...
  const handleWorkerSubmit = async () => {
    ...
    try {
      await api.post("/user-flow/worker-apply", { ...workerForm, userId: user.id });
      showToast("Application submitted!");
      setWorkerForm(INIT_WORKER);
      ...
      <PageContainer title={t("manage_addresses")} subtitle={t("manage_delivery")}>
        ...
        <div className="flex items-center justify-between mb-1">
          <span className="text-xs text-gray-400 font-medium">
            {addresses.length} {addresses.length !== 1 ? t("saved_addresses_plural") : t("saved_addresses")}
          </span>
          <PrimaryButton onClick={() => setShowAddAddress(v => !v)} className="text-xs px-3 py-2">
            <Plus size={13} />
            {showAddAddress ? t("cancel") : t("add_new")}
          </PrimaryButton>
        </div>
        ...
        {addrLoading ? (
          <div className="space-y-3">
            {[1, 2].map(i => <div key={i} className="rounded-2xl border border-gray-100 bg-white p-4 animate-pulse h-24" />)}
          </div>
        ) : addresses.length === 0 ? (
          <Card>
            <EmptyState icon="📍" title={t("no_addresses")} description={t("no_addresses_desc")} />
          </Card>
        ) : (
                  ...
                  <div>
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {addr.primaryAddress && (
                              <Badge tone="indigo">
                                <Star size={9} className="mr-1 fill-indigo-600" />
                                {t("primary")}
                              </Badge>
                            )}
                          </div>
                    ...
                    <div className="mt-3 flex items-center gap-2 pt-2 border-t border-gray-100/80">
                      ...
                      <div className="flex items-center gap-1 ml-auto">
                        <button onClick={() => setEditingAddress(addr)} className="flex h-7 w-7 items-center justify-center rounded-lg bg-gray-100 hover:bg-indigo-50 hover:text-indigo-600 transition-colors">
                          <Pencil size={12} />
                        </button>
                        <button onClick={() => handleDeleteAddress(addr.id)} className="flex h-7 w-7 items-center justify-center rounded-lg bg-gray-100 hover:bg-red-50 hover:text-red-600 transition-colors">
                          <Trash2 size={12} />
                        </button>
                      </div>
                    </div>
        ...
        <SecondaryButton onClick={() => { setView("profile"); navigate("/profile"); }} className="mt-2">
          {t("back_to_profile")}
        </SecondaryButton>
      </PageContainer>
    );
  ...
  return (
    <PageContainer>
      ...
      <div className="rounded-2xl gradient-primary p-5 text-white shadow-lg shadow-indigo-200">
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            ...
            <span className="mt-1.5 inline-flex rounded-full bg-white/20 px-2.5 py-0.5 text-[10px] font-bold text-white border border-white/20">
              {user?.role || "GUEST"}
            </span>
          </div>
          {isLoggedIn && (
      ...
      <div className="grid grid-cols-2 gap-2.5 stagger-children">
        ...
        {user?.role === "WORKER" && (
          <ProfileMenuItem icon={<Briefcase size={16} />} label={t("worker_dashboard")} desc={t("your_jobs")} onClick={() => navigate("/worker/dashboard")} color="bg-indigo-50 text-indigo-600" />
        )}
        {user?.role === "ADMIN" && (
          <ProfileMenuItem icon={<ShieldCheck size={16} />} label={t("admin_panel")} desc={t("manage_platform")} onClick={() => navigate("/admin/dashboard")} color="bg-rose-50 text-rose-600" />
        )}
      </div>
      ...
      {user?.role === "USER" && (
        <div>
          <button
            onClick={() => setShowWorkerForm((v) => !v)}
            className="w-full rounded-2xl border-2 border-dashed border-indigo-200 bg-indigo-50/50 p-4 text-center hover:bg-indigo-50 transition-colors"
          >
      ...
      {!isLoggedIn && (
        <button
          ...
          className="w-full rounded-2xl gradient-primary p-4 text-center text-sm font-bold text-white shadow-sm shadow-indigo-200 hover:opacity-90 transition-opacity"
        >
...
function ProfileMenuItem({ icon, label, desc, onClick, color }) {
  return (
    <button
      onClick={onClick}
      className="group flex items-center gap-3 rounded-2xl border border-gray-100 bg-white p-3.5 text-left shadow-sm hover:shadow-md hover:border-indigo-100 hover:-translate-y-0.5 transition-all duration-200 active:scale-[0.98]"
    >
