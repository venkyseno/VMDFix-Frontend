  ...
  const CaseCard = ({ c }) => (
    <div className={`rounded-xl border p-3.5 ${!c.workerId ? "border-amber-100 bg-amber-50/30" : "border-gray-100 bg-gray-50"}`}>
      <div className="flex items-center justify-between mb-2">
        <p className="font-bold text-gray-900 text-sm">Case #{c.id}</p>
      ...
      {!c.workerId && (
        <SelectField className="mt-2.5" onChange={async e => {
    ...
    <PageContainer title="Works" subtitle="Track orders and assign workers">
      <div className="grid gap-2 sm:grid-cols-2 text-center mb-2">
        <div className="rounded-2xl border border-amber-100 bg-amber-50 p-3">
          <p className="text-xl font-bold text-amber-700">{unassigned.length}</p>
          <p className="text-xs text-amber-600 font-semibold">Unassigned</p>
        </div>
        <div className="rounded-2xl border border-indigo-100 bg-indigo-50 p-3">
          <p className="text-xl font-bold text-indigo-700">{assigned.length}</p>
          <p className="text-xs text-indigo-600 font-semibold">Assigned</p>
        </div>
      </div>

      <Card>
        <SectionHeader title="Unassigned Cases" />
        {unassigned.length === 0 ? <EmptyState icon="✅" title="All cases assigned!" /> : (
          <div className="space-y-2">{unassigned.map(c => <CaseCard key={c.id} c={c} />)}</div>
        )}
      </Card>

      <Card>
        <SectionHeader title="Assigned Cases" />
        {assigned.length === 0 ? <EmptyState icon="📋" title="No assigned cases" /> : (
          <div className="space-y-2">{assigned.map(c => <CaseCard key={c.id} c={c} />)}</div>
        )}
      </Card>
    </PageContainer>
  );
