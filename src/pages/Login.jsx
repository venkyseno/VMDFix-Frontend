...
export default function Login() {
  ...
  const submit = async () => {
    ...
    try {
      if (authType === "login") { const { data } = await api.post("/users/login", { mobile: form.mobile, password: form.password }); return postAuth(data); }
      if (method === "email") { const { data } = await api.post("/users/signup", { name: form.name, email: form.email, mobile: form.mobile, password: form.password, signupProvider: "EMAIL" }); return postAuth(data); }
      if (method === "google") { const { data } = await api.post("/users/google-signup", { name: form.name, email: form.email, mobile: form.mobile }); return postAuth(data); }
      const { data } = await api.post("/users/mobile-otp-signup", { name: form.name, mobile: form.mobile, otp: form.otp });
      postAuth(data);
    } catch (e) { setError(e.response?.data?.message || e.response?.data || t("auth_failed")); }
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 py-10">
        <div className="w-full max-w-sm">
          ...
          <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
            ...
            {error && <div className="mt-3 rounded-xl bg-red-50 border border-red-100 px-3 py-2"><p className="text-xs font-semibold text-red-600">{error}</p></div>}
