...
import {
  Badge, Card, DangerButton, EmptyState, InputField, PageContainer,
  PrimaryButton, SectionHeader, SelectField, SecondaryButton, TextAreaField, GhostButton
} from "../../components/ui";
import { Image, Plus, ShoppingBag, Pencil, Trash2, ChevronDown, ChevronUp } from "lucide-react";
...
export default function AdminBannersPage() {
  ...
  const [activeTab, setActiveTab] = useState("banners"); // "banners" | "services"

  const err = (e) => alert(e?.response?.data || e?.message || "Request failed");

  const load = async () => {
    const [b, s] = await Promise.allSettled([api.get("/admin/banners"), api.get("/admin/other-services")]);
    if (b.status === "fulfilled") setBanners(b.value.data || []); else { setBanners([]); err(b.reason); }
    if (s.status === "fulfilled") setOtherServices(s.value.data || []); else { setOtherServices([]); err(s.reason); }
  };
  ...
  const svcOpts = useMemo(() => services.map(s => ({ id: String(s.id), label: `${s.id} · ${s.name}` })), []);
  ...
  const saveBanner = async () => {
    ...
    if (["SERVICE","OTHER_SERVICE"].includes(bannerForm.redirectType) && !bannerForm.targetId) return alert("Select a target");
    try {
  ...
  const updateBanner = async () => {
    ...
    if (!editingBanner.title?.trim()) return alert("Banner title is required");
    if (!editingBanner.imageUrl?.trim()) return alert("Banner image is required");
    // Send only fields the backend expects - strip frontend-only fields
    const payload = {
      title: editingBanner.title,
      imageUrl: editingBanner.imageUrl,
      redirectPath: buildPath(editingBanner.redirectType, editingBanner.targetId),
  ...
  const saveService = async () => {
    ...
    try { await api.post("/admin/other-services", serviceForm); setServiceForm(EMPTY_SERVICE); setShowServiceForm(false); load(); }
    catch(e) { err(e); }
  };

  const updateService = async () => {
    if (!editingService) return;
    if (!editingService.name?.trim()) return alert("Name required");
    if (!editingService.imageUrl?.trim()) return alert("Image required");
    try { await api.put(`/admin/other-services/${editingService.id}`, editingService); setEditingService(null); load(); }
    catch(e) { err(e); }
  };
  ...
  const addItem = async () => {
    ...
    if (!itemForm.price) return alert("Price required");
    if (!itemForm.availableQuantity) return alert("Quantity required");
    try {
    ...
    <PageContainer title="Content Management" subtitle="Manage banners and marketplace services">
      ...
      <div className="flex rounded-2xl bg-gray-100 p-1 gap-1">
        {[
          ...
          { key:"services", label:"🛒 Other Services", count: otherServices.length }
        ].map(tab => (
          <button
            key={tab.key}
      ...
      {activeTab === "banners" && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-sm font-bold text-gray-900">Banner Management</h2>
              <p className="text-xs text-gray-400">Promotional banners on the home screen</p>
            </div>
            <PrimaryButton onClick={() => setShowBannerForm(v => !v)}>
              <Plus size={14} />
          ...
          {showBannerForm && (
            <Card className="border-indigo-100 animate-scale-in">
              <SectionHeader title="New Banner" subtitle="Fill in the details below" />
              <div className="space-y-4">
                <div className="grid gap-3 sm:grid-cols-2">
                  ...
                  <InputField label="Display Duration (seconds)" type="number" value={bannerForm.displaySeconds} onChange={e => setBannerForm({ ...bannerForm, displaySeconds:e.target.value })} />
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  ...
                  {bannerForm.redirectType === "SERVICE" && (
                    <SelectField label="Target Service" value={bannerForm.targetId} onChange={e => setBannerForm({ ...bannerForm, targetId:e.target.value })}>
                      ...
                      {svcOpts.map(o => <option key={o.id} value={o.id}>{o.label}</option>)}
                    </SelectField>
                  )}
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3">
                    <input type="file" accept="image/*" className="flex-1" onChange={async (e) => {
                      const file = e.target.files?.[0];
                      if (!file) return;
                      if (file.size > 20*1024*1024) return alert("File too large (max 20MB)");
                      setUploadingBanner(true);
                      try { const url = await uploadFile(file); setBannerForm(p => ({ ...p, imageUrl:url })); }
                      catch(e) { err(e); } finally { setUploadingBanner(false); }
                    }} />
                    {uploadingBanner && <span className="text-xs text-gray-400 animate-pulse">Uploading...</span>}
                  </div>
                  {bannerForm.imageUrl && (
                    <img src={bannerForm.imageUrl} alt="preview" className="mt-2 h-24 w-full rounded-xl object-cover border border-gray-100" />
                  )}
                </div>
          ...
          <Card>
            ...
            {banners.length === 0 ? (
              <EmptyState icon="🖼️" title="No banners published" description="Create your first banner above." />
            ) : (
                ...
                {banners.map((banner) => (
                  <div key={banner.id}>
                    <div className="flex items-center gap-3 rounded-xl border border-gray-100 bg-gray-50 p-3">
                      {banner.imageUrl && (
                        <img src={banner.imageUrl} alt="" className="h-14 w-20 rounded-lg object-cover flex-shrink-0" />
                      )}
                      ...
                      <div className="flex gap-1.5 flex-shrink-0">
                        <button
                          onClick={() => { const p = parsePath(banner.redirectPath); setEditingBanner({ ...banner, ...p, displaySeconds:banner.displaySeconds||5, placement:banner.placement||"HOME" }); }}
                          className="h-8 w-8 flex items-center justify-center rounded-xl bg-white border border-gray-200 hover:border-indigo-200 hover:text-indigo-600 transition-all"
                        >
                          <Pencil size={13} />
                        </button>
                        <button
                          onClick={async () => { if(!confirm("Delete banner?")) return; await api.delete(`/admin/banners/${banner.id}`); if(editingBanner?.id===banner.id) setEditingBanner(null); load(); }}
                      ...
                      <div className="mt-2 rounded-xl border border-indigo-100 bg-indigo-50/40 p-4 animate-scale-in">
                        ...
                        <div className="grid gap-3 sm:grid-cols-2">
                          ...
                          {editingBanner.redirectType === "SERVICE" && (
                            <SelectField label="Target Service" value={editingBanner.targetId||""} onChange={e => setEditingBanner({ ...editingBanner, targetId:e.target.value })}>
                        ...
                        <div className="mt-3">
                          <input type="file" accept="image/*" onChange={async (e) => {
                            ...
                            try { const url = await uploadFile(file); setEditingBanner(p => ({ ...p, imageUrl:url })); } catch(e) { err(e); }
                          }} />
      ...
      {activeTab === "services" && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-sm font-bold text-gray-900">Marketplace Services</h2>
              <p className="text-xs text-gray-400">Menu-based services with item pricing</p>
            </div>
            <PrimaryButton onClick={() => setShowServiceForm(v => !v)}>
              <Plus size={14} />
          ...
          {showServiceForm && (
            <Card className="border-indigo-100 animate-scale-in">
              ...
              <div className="space-y-3">
                ...
                <TextAreaField label="Description" rows={3} placeholder="Describe the service..." value={serviceForm.menuDetails} onChange={e => setServiceForm({ ...serviceForm, menuDetails:e.target.value })} />
                <div>
                  ...
                  <input type="file" accept="image/*" onChange={async (e) => {
                    ...
                    try { const url = await uploadFile(file); setServiceForm(p => ({ ...p, imageUrl:url })); }
                    catch(e) { err(e); } finally { setUploadingService(false); }
                  }} />
                ...
                <div className="flex gap-2">
                  <PrimaryButton onClick={saveService}>Publish Service</PrimaryButton>
                  <SecondaryButton onClick={() => setShowServiceForm(false)}>Cancel</SecondaryButton>
                </div>
              </div>
            </Card>
          )}
          ...
          <Card>
            ...
            {otherServices.length === 0 ? (
              <EmptyState icon="🛒" title="No services yet" description="Add your first marketplace service above." />
            ) : (
              <div className="space-y-2">
                {otherServices.map((s) => (
                  <div key={s.id}>
                    <div className="flex items-center gap-3 rounded-xl border border-gray-100 bg-gray-50 p-3">
                      {s.imageUrl && (
                        <img src={s.imageUrl} alt="" className="h-14 w-20 rounded-lg object-cover flex-shrink-0" />
                      )}
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                          <p className="font-bold text-gray-900 text-sm">{s.name}</p>
                          <Badge tone="gray">#{s.id}</Badge>
                        </div>
                      ...
                      <div className="flex gap-1.5">
                        <button
                          onClick={() => { setSelectedServiceId(String(s.id)); setEditingService(s); }}
                          className="h-8 w-8 flex items-center justify-center rounded-xl bg-white border border-gray-200 hover:border-indigo-200 hover:text-indigo-600 transition-all"
                        >
                          <Pencil size={13} />
                        </button>
                        <button
                          onClick={async () => { if(!confirm("Delete service?")) return; await api.delete(`/admin/other-services/${s.id}`); load(); }}
                          className="h-8 w-8 flex items-center justify-center rounded-xl bg-white border border-red-100 hover:bg-red-50 hover:text-red-600 transition-all"
                        >
                          <Trash2 size={13} />
                        </button>
                    ...
                    {editingService?.id === s.id && (
                      <div className="mt-2 rounded-xl border border-indigo-100 bg-indigo-50/30 p-4 space-y-4 animate-scale-in">
                        ...
                        <div>
                          <input type="file" accept="image/*" onChange={async (e) => {
                            const file = e.target.files?.[0]; if(!file) return;
                            try { const url = await uploadFile(file); setEditingService(p => ({ ...p, imageUrl:url })); } catch(e) { err(e); }
                          }} />
                        ...
                        <div className="flex gap-2">
                          ...
                          <SecondaryButton onClick={() => setEditingService(null)}>Cancel</SecondaryButton>
                        </div>
                            ...
                            {items.map(item => (
                              <div key={item.id} className="flex items-center justify-between rounded-xl bg-white border border-gray-100 px-3 py-2">
                                <div>
                                  ...
                                  <span className="text-xs text-gray-400 ml-2">₹{item.price} · Qty {item.availableQuantity}</span>
                                </div>
                                <button
                                  onClick={async () => {
                                  ...
                                  className="h-7 w-7 flex items-center justify-center rounded-lg hover:bg-red-50 hover:text-red-600 text-gray-400 transition-all"
                                >
                                  <Trash2 size={12} />
                                </button>
                              </div>
